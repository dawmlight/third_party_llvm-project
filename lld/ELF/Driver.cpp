...
static void checkOptions() {
  ...
  if (!config->combineEhSections && config->ehFrameHdr) {
    error(
        "-no-combine-eh-sections and -eh-frame-hdr may not be "
        "used together"
    );
  }
...
static void readConfigs(opt::InputArgList &args) {
  ...
  config->combineEhSections =
      args.hasFlag(OPT_combine_eh_sections, OPT_no_combine_eh_sections, true);
