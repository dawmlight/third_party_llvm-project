...
//
//===----------------------------------------------------------------------===//
// 2021.7.19 Add OHOS target support for lld
...
StringRef getOutputSectionName(const InputSectionBase *s) {
  ...
  // Process -no-combine-eh-sections option
  // Those changes splits single loop of section prefix lookup onto
  // generic sections loop and EH sections loop.
  auto GenericSections = {
      ".text.", ".rodata.", ".data.rel.ro.", ".data.", ".bss.rel.ro.", ".bss.",
      ".init_array.", ".fini_array.", ".ctors.", ".dtors.", ".tbss.", ".tdata.",
      ".gcc_except_table."
  };
  for (StringRef v : GenericSections)
    if (isSectionPrefix(v, s->name))
      return v.drop_back();

  if (config->combineEhSections) {
    // Note .gcc_except_table. is not a regular EH section. It stores LSDA
    // records, used by personality routines. Those sections are referenced from
    // personality routines by regular pointers.
    // It is still possible to prevent such sections from merging by
    // explicit SECTION command in linker script. Otherwise such sections
    // become an orphan sections and handled by this method as generic ones.
    auto EhSections = {".ARM.extab.", ".ARM.exidx."};
    for (StringRef v : EhSections) {
      if (isSectionPrefix(v, s->name))
        return v.drop_back();
    }
  }
  // end of Process -no-combine-eh-sections option
...
template <class ELFT> void createSyntheticSections() {
  ...
  for (Partition &part : partitions) {
    ...
    if (config->combineEhSections && !config->relocatable) {
...
std::vector<PhdrEntry *> Writer<ELFT>::createPhdrs(Partition &part) {
  ...
  if (part.ehFrame && part.ehFrame->isNeeded() && part.ehFrameHdr &&
