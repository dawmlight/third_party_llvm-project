...
static std::string getImplibPath(bool useDllPathForLib) {
  SmallString<128> out = StringRef(config->outputFile);
  if (!config->implib.empty()) {
    if (useDllPathForLib) {
      SmallString<128> result = sys::path::parent_path(out);
      sys::path::append(result, config->implib);
      return result.str();
    }
    return config->implib.str();
  }
...
static void createImportLibrary(bool asLib, bool useDllPathForLib) {
  ...
  std::string path = getImplibPath(useDllPathForLib);
...
void LinkerDriver::link(ArrayRef<const char *> argsArr) {
  ...
  if (!args.hasArg(OPT_INPUT, OPT_wholearchive_file)) {
    fixupExports();
    createImportLibrary(/*asLib=*/true, args.hasArg(OPT_libusedllpath));
    return;
  }
  ...
  if (!config->exports.empty() || config->dll) {
    ...
    if (!config->mingw || !config->implib.empty())
      createImportLibrary(/*asLib=*/false, args.hasArg(OPT_libusedllpath));
