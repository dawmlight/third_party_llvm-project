...
// to get the pointer to thread-specific data keys in the thread control block.
    ...
    (SANITIZER_FREEBSD || SANITIZER_LINUX) && !SANITIZER_BIONIC
...
uptr ThreadDescriptorSize() {
  ...
  int patch;
  if (GetLibcVersion(&major, &minor, &patch) && major == 2) {
    // TODO(smd): consider adding an optional runtime check for an unknown
    // (untested) glibc version
...
static void GetTls(uptr *addr, uptr *size) {
  ...
  *addr = 0;
  *size = 0;
#elif SANITIZER_ANDROID || SANITIZER_OHOS
  *addr = 0;
  *size = 0;
