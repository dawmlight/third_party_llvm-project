...
const unsigned struct_kernel_stat_sz = (SANITIZER_ANDROID || SANITIZER_OHOS)
...
#if SANITIZER_BIONIC && defined(_LP64)
  int data[4];
#elif SANITIZER_BIONIC && !defined(_LP64)
  int data;
...
#if SANITIZER_LINUX && !SANITIZER_BIONIC
...
#if SANITIZER_BIONIC && (SANITIZER_WORDSIZE == 64)
...
#elif SANITIZER_BIONIC && (SANITIZER_WORDSIZE == 32)
struct __sanitizer_sigaction {
  union {
    __sanitizer_sigactionhandler_ptr sigaction;
    __sanitizer_sighandler_ptr handler;
...
struct __sanitizer_addrinfo {
  ...
  int ai_socktype;
  int ai_protocol;
#if SANITIZER_BIONIC || SANITIZER_MAC
  unsigned ai_addrlen;
  char *ai_canonname;
  void *ai_addr;
#else // LINUX
