...
#define SCUDO_TSD_EXCLUSIVE_SUPPORTED (!SANITIZER_ANDROID && \
                                       !SANITIZER_FUCHSIA && \
                                       !SANITIZER_OHOS)

#ifndef SCUDO_TSD_EXCLUSIVE
// SCUDO_TSD_EXCLUSIVE wasn't defined, use a default TSD model for the platform.
# if SANITIZER_ANDROID || SANITIZER_FUCHSIA || SANITIZER_OHOS
...
# if defined(__aarch64__) && (SANITIZER_ANDROID || SANITIZER_OHOS)
...
# endif
#else
const uptr RegionSizeLog = (SANITIZER_ANDROID || SANITIZER_OHOS) ? 19 : 20;
