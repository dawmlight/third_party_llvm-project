...
PlatformSP PlatformOHOS::CreateInstance(bool force, const ArchSpec *arch) {
  ...
  if (!create && arch && arch->IsValid()) {
...
#if defined(__OHOS__)
    // Only accept "unknown" for the vendor if the host is ohos and if
    ...
    if (create) {
      ...
      case llvm::Triple::OHOS:
        break;

#if defined(__OHOS__)
      // Only accept "unknown" for the OS if the host is ohos and it
      // "unknown" wasn't specified (it was just returned because it was NOT
      // specified)
...
const char *PlatformOHOS::GetPluginDescriptionStatic(bool is_host) {
  if (is_host)
    return "Local OHOS user platform plug-in.";
  else
    return "Remote OHOS user platform plug-in.";
}
...
Status PlatformOHOS::DownloadSymbolFile(const lldb::ModuleSP &module_sp,
                                           const FileSpec &dst_file_spec) {
...
HdcClient::SyncService *PlatformOHOS::GetSyncService(Status &error) {
  if (m_hdc_sync_svc && m_hdc_sync_svc->IsConnected())
    return m_hdc_sync_svc.get();

  HdcClient hdc(m_device_id);
  m_hdc_sync_svc = hdc.GetSyncService(error);
  return (error.Success()) ? m_hdc_sync_svc.get() : nullptr;
}
