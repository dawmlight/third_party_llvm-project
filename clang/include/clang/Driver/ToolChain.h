...
public:
  ...
  std::string getMultiarchTripleForPath(llvm::Triple T) const {
    const bool IsLiteOS = T.isOSLiteOS();
    auto GetTripleWithPostfix = T.isOHOSMusl() ?
      [](const char* name) { return std::string(name) + "musl"; }
    ...
    case llvm::Triple::thumb:
      if (IsLiteOS) {
        ...
        return GetTripleWithPostfix("arm-linux-ohos");
      }
    ...
    case llvm::Triple::aarch64:
      return GetTripleWithPostfix("aarch64-linux-ohos");
    }
    return T.str();
  }
