...
void tools::gnutools::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  const bool isOHOS = ToolChain.getTriple().isOHOS();
  const bool isLiteOS = ToolChain.getTriple().isOSLiteOS();
  const bool isOHOSMusl = ToolChain.getTriple().isOHOSMusl();
  // OHOS use the same crt obj files as Android.
  const bool isAndroid = ToolChain.getTriple().isAndroid() ||
                          (isOHOS && !isLiteOS && !isOHOSMusl);
  ...
  if (Arch == llvm::Triple::aarch64 && (isAndroid || isOHOS)) {
  ...
  if (!Args.hasArg(options::OPT_nostdlib)) {
    if (!Args.hasArg(options::OPT_nodefaultlibs)) {
      ...
      if (WantPthread && !isAndroid && !isOHOS)
...
/*static*/ void Generic_GCC::GCCInstallationDetector::CollectLibDirsAndTriples(
  ...
  // OHOS targets should not use GNU/Linux tools or libraries.
  if (TargetTriple.isOHOS()) {
    static const char *const AArch64OHOSTriples[] = {"aarch64-linux-ohos"};

    switch (TargetTriple.getArch()) {
    case llvm::Triple::aarch64:
      LibDirs.append(begin(AArch64LibDirs), end(AArch64LibDirs));
      TripleAliases.append(begin(AArch64OHOSTriples),
                           end(AArch64OHOSTriples));
      break;
    default:
      break;
    }
