...
Optional<std::string> ToolChain::getRuntimePath() const {
  ...
  if (Triple.isOHOS()) {
    P.assign(D.ResourceDir);
    llvm::sys::path::append(P, "lib", getMultiarchTripleForPath(Triple));
    if (getVFS().exists(P))
      return llvm::Optional<std::string>(P.str());
  }
...
Optional<std::string> ToolChain::getCXXStdlibPath() const {
  ...
  if (Triple.isOHOS()) {
    P.assign(D.Dir);
    llvm::sys::path::append(P, "..", "lib", getMultiarchTripleForPath(Triple), "c++");
    if (getVFS().exists(P))
      return llvm::Optional<std::string>(P.str());
  }
...
std::string ToolChain::getArchSpecificLibPath() const {
  ...
  llvm::Triple Triple = getTriple();
  if (Triple.isOHOS()) {
    llvm::sys::path::append(Path, "lib", getMultiarchTripleForPath(Triple));
  } else {
    llvm::sys::path::append(Path, "lib", getOSLibName(),
